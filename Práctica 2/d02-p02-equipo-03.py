...
class Process:
    ...
    def random_color(self):
        ...
        color = [x.replace('0x', '') if len(
            x) == 4 else x.replace('x', '') for x in color]
        return '#' + ''.join(color)
...
class Graph:
    ...
    def draw_bar(self, i):
        for p in self._processes:
            if i >= p._init:
                self._ax.barh(p._id, p._age, color=p._color,
                              capstyle='round', left=p._init)
                p.run()
        plt.pause(0.01)
...
class ProcessWindow:
    ...
    def setup_window(self):
        ...
        self._title_frame = tk.Frame(
            self._root, background="#F0F0F0", relief="flat")
        self._graph_frame = tk.Frame(
            self._root, background="#F0F0F0", relief="sunken")
        self._table_frame = tk.Frame(
            self._root, background="#eeeeee", relief="sunken")
        self._button_frame = tk.Frame(
            self._table_frame, background="#F0F0F0", relief="flat")
    ...
    def setup_button(self):
        self._back_button = tk.Button(
            self._button_frame, font='Helvetica 32', text='Regresar', command=self.quit)
    ...
    def create_processes_table(self):
        ...
        x = 0
        y = 28
        tk.Label(self._table_frame, text='Tabla de procesos',
                 font=('Arial', 15, 'bold')).place(x=x, y=0)
        for j in range(3):
            p_table = tk.Entry(self._table_frame, width=12,
    ...
    def create_ordered_processes_table(self):
        ...
        x = 0
        y = self._y + 72
        tk.Label(self._table_frame, text='Tabla de procesos en orden de ejecución', font=(
            'Arial', 15, 'bold')).place(x=x, y=y)
        y += 28
        for j in range(3):
            p_table = tk.Entry(self._table_frame, width=12,
                               fg='black', font=('Arial', 15, 'bold'))
    ...
    def time_label(self):
        ...
        time_label = tk.Label(self._table_frame, text=str(
            longest[-1]._death), font=('Arial', 35, 'bold'))
    ...
    def animate(self):
        ...
        self._processes = [Process(i, elem[0], elem[1])
                           for i, elem in enumerate(self._processes)]
...
class SetupWindow:
    ...
    def setup_window(self):
        ...
        self._input_frame = tk.Frame(
            self._root, background="#F0F0F0", relief="flat")
        self._confirmation_frame = tk.Frame(
            self._root, background="#F0F0F0", relief="flat")
        # Se define la posición de los frames
        self._input_frame.grid(row=0, column=0, sticky="nsew", padx=2, pady=0)
        self._confirmation_frame.grid(
            row=1, column=0, sticky="nsew", padx=2, pady=0)
        # Definimos la estructura de la ventana principal: Filas y columna
    ...
    def define_process_widget(self):
        #
        self._init_labels.append(
            tk.Label(self._input_frame, text='Inicio', font='15'))
        self._init_text_inputs.append(tk.Entry(self._input_frame, width=9))
        self._duration_labels.append(
            tk.Label(self._input_frame, text='Duración', font='15'))
        self._duration_text_inputs.append(tk.Entry(self._input_frame, width=9))
        #
        self._add_buttons.append(
            tk.Button(self._input_frame, text='+', command=self.add_process_row))
    ...
    def add_process_row(self):
        if self._count_row < 8:
            ...
            self.define_process_widget()
            self.define_process_pos(
                x_pos=x, y_pos=y+33, x_button=330, y_button=y+30)
            self._count_row += 1
        else:
            tk.messagebox.showerror(
                title='Error', message='Limite de procesos alcanzado')
    ...
    def run_processes(self):
        while True:
            if self.verify_all_field():
                processes = zip(self._init_text_inputs,
                                self._duration_text_inputs)
                processes = [(int(init.get()), int(duration.get()))
                             for init, duration in processes]
                # ordenarlos
    ...
    def is_valid(self):
        ...
        for i, init in enumerate(self._init_text_inputs):
            try:
                state, error, field, no = self.verify_field(
                    init, 'Inicio', i+1)
    ...
    def verify_all_field(self):
        ...
        if state:
            for i, (init, duration) in enumerate(zip(self._init_text_inputs, self._duration_text_inputs)):
                try:
                    state, error, no = self.is_valid_number(
                        init, duration, i+1)
